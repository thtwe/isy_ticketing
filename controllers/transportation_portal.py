...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        domain = [('key_type', '=', 'transportation'), '|', '|', ('create_uid', '=', request.env.user.id), ('user_ids', 'in', [request.env.user.id]), ('driver_id', '=', request.env.user.id)]
        transportation_requests_count = request.env['isy.ticketing.requests'].search_count(domain)
        values['transportation_requests_count'] = transportation_requests_count
        return values

    @http.route(['/my/transportation_requests', '/my/transportation_requests/page/<int:page>'], type='http', auth="user", website=True)
    def portal_transportation_requests(self, page=1, sortby=None, filterby=None, search=None, search_in='all', **kw):
        domain = []
        ...
        # count for pager
        transportation_requests_count = IsyTicketingRequests.sudo().search_count(domain)
        # pager

        pager = portal_pager(
            url="/my/transportation_requests",
            url_args={'sortby': sortby},
            total=transportation_requests_count,
            page=page,
            step=self._items_per_page
        )
        # content according to pager and archive selected
        transportation_requests = IsyTicketingRequests.sudo().search(
            domain, order=order, limit=self._items_per_page, offset=pager['offset'])
        request.session['my_transportation_requests_history'] = transportation_requests.ids[:100]

        values.update({
            'transportation_requests': transportation_requests,
            'page_name': 'transportation_request',
            'pager': pager,
            'default_url': '/my/transportation_requests',
        ...
        return request.render("isy_ticketing.portal_my_transportation_requests", values)
